(() => {
  ...
  function sampleArrow(){const span=document.createElement('span');span.innerHTML='<svg class="ui-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 18 18 6M6 6h12v12"/></svg>';return span;}
  const examples = {
  ...
  function choose(key){token++;running=false;current=key;const c=examples[key];const n=counts[key] ||= {count:c.count,revenue:c.revenue,customers:c.customers};
    ...
    const button=$('#sample-order');button.disabled=false;button.replaceChildren(document.createTextNode(c.action+' '),sampleArrow());
